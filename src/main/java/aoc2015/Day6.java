...
public class Day6 {
    ...
    static int solve1(final String input) {
        ...
        final BitSet[] grid = new BitSet[n];
        for (int i = 0; i < n; i++)
            grid[i] = new BitSet();

        final Pattern numsPattern = Pattern.compile("\\d+");
        for (final String instruction : input.split("\n")) {
            Matcher m = numsPattern.matcher(instruction);

            m.find();
            final int top = Integer.valueOf(m.group());
            m.find();
            final int left = Integer.valueOf(m.group());
            m.find();
            final int bottom = Integer.valueOf(m.group());
            m.find();
            final int right = Integer.valueOf(m.group());

            boolean on = instruction.startsWith("turn on");
            boolean off = instruction.startsWith("turn off");

            for (int row = top; row <= bottom; row++) {
                for (int col = left; col <= right; col++) {
                    // grid[row][col] = on ? 1 : off ? 0 : (byte) (grid[row][col] ^ 1);
                    if (on)
                        grid[row].set(col);
                    else if (off)
                        grid[row].clear(col);
    ...
    static int solve2(final String input) {
        ...
        for (final String instruction : input.split("\n")) {
            ...
            for (int row = top; row <= bottom; row++)
                for (int col = left; col <= right; col++)
                    grid[row][col] += on ? 1 : off ? grid[row][col] > 0 ? -1 : 0 : 2;
        }
